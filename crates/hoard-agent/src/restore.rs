...
/// covers both.
///
/// Since the share, only the list has gone up, the owner's included, and the
/// pre-restore safety copy walks the same list: without this, a character
/// played after the share and overwritten by an older version would be in no
/// version and in no copy. `share_include` comes from
/// [`crate::savefilter::owner_share_include`]; empty, and `None` back, for any
/// other restore. Files are copied, never moved: the restore decides what
...
mod tests {
    ...
    /// The owner shared world One and kept playing a character; since the
    /// share only the world went up. Restoring an older version writes the
